...
public class FFmpegQueryExtension {
    ...
    /*fun addTextandWMOnVideo(inputVideo: String,imageInput:String, textInput: String, posX: Float?, posY: Float?, fontPath: String, isTextBackgroundDisplay: Boolean, fontSize: Int, fontcolor: String, output: String): Array<String> {
        val inputs: ArrayList<String> = ArrayList()
        var borderQuery = ""
        ...
        inputs.apply {
            ...
            add(imageInput)
            add("-filter_complex")
            add("[0:v][1:v]overlay=10:10,drawtext=text='Hello World'")
            add("-c:a")
            add("copy")
            add("-movflags")
            add("+faststart")
...
inputs.apply {
   ...
   add(inputVideo)
   add("-i")
   add("/storage/emulated/0/Movies/image.png")
   add("-filter_complex")
   add("[0:v][1:v]overlay=10:10,drawtext=text='Hello World'")
   add("-c:a")
   add("copy")
